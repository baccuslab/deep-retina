...
from convbatchnorm.normalization import ConvBatchNormalization, ConvAdaptiveNormalization
...
def bn_layer(x, nchan, size, l2_reg, sigma=0.05):
    ...
    #return Reshape((nchan, n, n))(BatchNormalization(axis=-1)(Flatten()(y)))
    return BatchNormalization(axis=1)(y)
...
def adaptive_bn_layer(x, nchan, size, l2_reg, sigma=0.05):
    """An individual convolutional batchnorm layer"""
    n = int(x.shape[-1]) - size + 1
    y = Conv2D(nchan, size, data_format="channels_first", kernel_regularizer=l2(l2_reg))(x)
    y = ConvAdaptiveNormalization(normalization_axis=[1, 2, 3], parameter_axis=[1], kernel_size=(5,5))(y)
    return Activation('relu')(GaussianNoise(sigma)(y))
...
def conv_bn_cnn(inputs, n_out, l2_reg=0.01):
    """Batchnorm CNN model"""
