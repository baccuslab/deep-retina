...
def subunit_slice(x, subunit_idx):
    '''Slices 5D tensor along subunit dimension'''
    return x[:, :, subunit_idx, :, :]

def invariant_subunit_slice(x, subunit_idx):
    '''Slices 5D tensor along subunit dimension, but preserves original dims'''
    y = x[:, :, subunit_idx, :, :]
    z = T.reshape(y, (x.shape[0], x.shape[1], 1, x.shape[2], x.shape[3]))
    return z
