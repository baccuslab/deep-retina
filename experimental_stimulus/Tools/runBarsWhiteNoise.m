function ex = runBarsWhiteNoise(ex)
%
% FUNCTION ex = runBarsWhiteNoise(ex)
%
% Runs a basic white noise bars stimulus, with parameters defined in the appropriate
...
while fi <= ex.pa.nFrames && ~ex.kb.keyCode(ex.kb.escKey)
	...
	tex = ex.ds.gray + ex.ds.gray .* ...
		...
		randn(ex.pa.random(si).stream, 1, ex.pa.nBoxes);
	tex = min(max(tex, ex.ds.black), ex.ds.white);
	texture(:, :, 1) = ones(ex.pa.nBoxes, 1) * round(tex);
